...
import { formatDate, formatWorkerRole } from "@/utils/helpers"
import Image from "next/image"
import { useWorkersStore } from "../../store"

function WorkersTable() {
  const data = useWorkersStore((state) =>
    state.data
      ?.sort((a, b) => {
        const nameA = a.name.toLowerCase()
        const nameB = b.name.toLowerCase()
        const dateA = formatDate(a.birthDate)
        const dateB = formatDate(b.birthDate)
      ...
      .filter((user) => {
        ...
        const jobTitle = formatWorkerRole(user.role).toLowerCase().trim()
        const date = formatDate(user.birthDate)
          ...
          {data &&
            ...
            data.map((user) => (
              <Table.Row key={user.id}>
                ...
                <Table.RowCell>{formatWorkerRole(user.role)}</Table.RowCell>
                <Table.RowCell>{formatDate(user.birthDate)}</Table.RowCell>
